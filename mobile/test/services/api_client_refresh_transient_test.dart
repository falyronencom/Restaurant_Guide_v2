...
void main() {
  ...
  group('Обновление не дошло — сессия жива', () {
    ...
    test('5xx на обновлении: ровно два предъявления токена, без лестницы',
        () async {
      ...
      // предъявления на два цикла — восемь, последнее на `7T+6`, и часть из
      // них выпадала за льготное окно бэкенда, а это 403 и отзыв всех сессий.
      //
      // Теперь цикл равен одному запросу, попытка с повтором — двум. Оба
      // уходят не позже таймаута приёма от ротации, то есть заведомо внутри
      // окна. Именно этот счётчик и держит границу: вернись лестница на путь
      // обновления — здесь станет 8.
      //
      // Тест перестал быть медленным: реальных пауз на этом пути больше нет.
      final adapter = StubAdapter(
      ...
      expect(refreshCount(adapter), 2,
          reason: 'попытка и её единственный повтор — лестницы повторов 5xx '
              'на пути обновления больше нет');
      expectSessionKept();
    });

    test('лестница 5xx цела для обычного запроса', () async {
      // Обратная сторона той же границы. Снят с лестницы ТОЛЬКО путь
      // обновления — там счёт предъявлений токена решает судьбу сессии. Любой
      // другой запрос повторяется как прежде: 502 от edge в окно деплоя
      // переживается дроблением пауз 0,5 / 1 / 1,5 с, и гость не видит
      // ошибки.
      //
      // Без этого стража ветку повторов можно было бы удалить целиком, и ни
      // один тест набора не покраснел бы: после 13.09 все прочие счётчики
      // лестницы стоят на пути обновления, где её больше нет.
      final adapter = StubAdapter(
        (options) => jsonBody({'success': false, 'message': 'Bad Gateway'},
            status: 502),
        maxRequests: 12,
      );
      final api = client(adapter);
      ...
      expect(adapter.requests.length, Environment.maxRetryAttempts + 1,
          reason: 'исходный запрос и три штатных повтора — и стоп; счётчик '
              'обязан переезжать в повтор, иначе предела нет');
    ...
    test('вход остаётся на лестнице 5xx', () async {
      ...
      // сессии. У входа и OAuth погашать нечего — 502 от edge обязан
      // переживаться тихим повтором.
      //
      // Верни сюда `_isCredentialRequest` вместо `_isRefreshRequest` — и
      // секундная просадка edge заставит пользователя вводить пароль заново,
      // а на OAuth выбросит уже пройденное согласие в браузере: повтор там
      // стоит последним шагом, после всего разговора с провайдером.
      final adapter = StubAdapter(
        (options) => jsonBody({'success': false, 'message': 'Bad Gateway'},
            status: 502),
        maxRequests: 12,
      ...
      expect(adapter.requests.length, Environment.maxRetryAttempts + 1,
          reason: 'вход повторяется как любой другой запрос');
    });
