...
import '../helpers/dart_source_scan.dart';

/// Семейство, которым текст РИСУЕТСЯ, — а не то, что написано в теме.
///
/// Шрифты панели объявлены семействами в pubspec с 15.09.2026 (SDL
/// CAT-C-1.3), и тема раздаёт Nunito Sans через `ThemeData.fontFamily` и
/// `.apply()` на textTheme. Но оба пути питают только textTheme. Кнопки,
/// вкладки и чипы берут стиль подписи из своих тем и ЗАМЕНЯЮТ им
/// унаследованный стиль, а не сливают с ним, — поэтому стиль без `fontFamily`
/// даёт подписи пустое семейство, и в браузере она рисуется запасным шрифтом
/// движка. Ошибки при этом нет нигде — ни analyze, ни прогон, ни ревью её не
/// показывают. Отсюда форма проверок: семейство читается из
/// `RenderParagraph`, где стиль уже собран так, как его нарисуют.
void main() {
  group('Все пятнадцать слотов textTheme — семейство тела', () {
    // Слотов display* в TextTheme темы нет: их даёт типографика Material по
    // умолчанию, и семейство им отдаёт только `ThemeData.fontFamily` —
    // `.apply()` их не касается. Слоты перечислены литералом, а не выведены
    // из темы: выведенный список согласился бы с тем, что тема раздаёт.
    // Платформы все: панель открывают в браузерах любых систем, а
    // типографика Material по умолчанию у каждой своя.
        ...
        for (final slot in slots.entries) {
          expect(
            ...
            reason: 'textTheme.${slot.key} разрешается в '
                '«${slot.value?.fontFamily}», а не в семейство тела',
          );
        }
      });
    }
  ...
  group('Подписи компонентов нарисованы семейством канона', () {
    final platforms = TargetPlatformVariant.all();

    testWidgets('кнопки — из темы и канонические стили', (tester) async {
      final buttons = <String, Widget>{
        ...
        'canonHeaderAction': OutlinedButton(
          onPressed: () {},
          style: AppTheme.canonHeaderAction(),
          child: const Text('canonHeaderAction'),
        ),
      };
      ...
      for (final button in buttons.entries) {
        ...
        expect(
          ...
          reason: 'подпись ${button.key} нарисована не семейством тела: '
              'стиль подписи кнопки задан без fontFamily, а кнопка ЗАМЕНЯЕТ '
              'им унаследованный стиль',
        );
      }
    }, variant: platforms);

    testWidgets('вкладки — активная и неактивная', (tester) async {
      await _pumpInBody(
        tester,
        const DefaultTabController(
          length: 2,
          child: TabBar(tabs: <Widget>[Tab(text: 'Активная'), Tab(text: 'Соседняя')]),
        ),
      );
      for (final label in <String>['Активная', 'Соседняя']) {
        expect(
          _renderedFamily(tester, label),
          AppTheme.fontBodyFamily,
          reason: 'вкладка «$label»: tabBarTheme.labelStyle / '
              'unselectedLabelStyle без fontFamily, и вкладка уходит на '
              'запасной шрифт',
        );
      }
    }, variant: platforms);

    testWidgets('подписи чипов', (tester) async {
      ...
      for (final chip in chips.entries) {
        ...
        expect(
          ...
          reason: 'chipTheme.labelStyle без fontFamily: он заменяет стиль '
              'подписи по умолчанию, и ${chip.key} уходит на запасной шрифт',
        );
      }
    }, variant: platforms);
    ...
    testWidgets('контроль: явное семейство остаётся своим', (tester) async {
      ...
      await _pumpInBody(
        ...
        Column(
          ...
          children: <Widget>[
            Text('Unbounded', style: AppTheme.unbounded(fontSize: 20)),
            const Text('Body'),
          ],
        ),
      );
      expect(_renderedFamily(tester, 'Unbounded'), AppTheme.fontDisplayFamily);
  ...
  group('Стиль, ЗАМЕНЯЮЩИЙ унаследованный, называет семейство сам', () {
    ...
    // 23.09.2026 рисовались запасным шрифтом кнопки шапки модерации и
    // карточки ошибки.
    ...
    test('у каждого такого стиля есть fontFamily', () {
      ...
      expect(
        ...
        reason: 'стиль в параметре, который ЗАМЕНЯЕТ унаследованный, не '
            'называет семейство — подпись уйдёт на запасной шрифт: '
...
/// Поднимает [child] в теле экрана под темой панели.
...
/// стили поля ввода), сюда не входят: там семейство приходит от темы.
/// `RichText` разобран отдельно: он не наследует ничего.
