...
GLOSS_MIN_PX = 120                # порог «глянец / плоский» по размеру файла
# 120, а не 180: правило — иконка на ДОМАШНЕМ экране всегда глянцевая, плоский
# остаётся служебным мелким размерам и вкладкам браузера. При 180 под чертой
# оставались 167 px (домашний экран iPad) и 162 px (Android hdpi), то есть два
# устройства владельца показывали бы РАЗНЫЙ материал — это и была настоящая
# неконсистентность. Ниже 120 глянец действительно перестаёт читаться: блик и
# градиент в грязь, тень съедает контур (лист sheet_3_threshold).
